...
// spawn only if not found in config
...
module.exports = function(rootPath){
  return function (req, res) {
    ...
    // quick n dirty
    let callPath = [] 
    callpath.forEach( str => {
      if (/^0x/.test(str)) callPath.push("'"+str+"'"); // TODO: improve regexp so only valid addresses match
      else callPath.push(str);
    })
    if (!scope || !task) return (res.send('Hello api!\n'+
      'Currently, awaiting 2+ parameters after /geth (1+ after /Ethereum )(\n'));

    const onUnfulfillment = () => { res.send ("no such script"); }
    const onSuccess = () => {
      //execFile('geth'
      console.log('running on path ', callpath)
      const l = [`--jspath`, `${rootPath}`, 
	`--preload`, `test/mod.js,${scope}/${task}.js`,
